...
final class VolumeStreamTVViewModel: ObservableObject {
    ...
    func refreshShell(downloadsEnabled: Bool) {
        ...
        performBridgeJsonRequest(
            call: { completion in
                TvosContentBridge.shared.fetchShellSnapshotJson(downloadsEnabled: downloadsEnabled, completionHandler: completion)
            },
    ...
    func search(query: String) {
        ...
        performBridgeJsonRequest(
            call: { completion in
                TvosContentBridge.shared.searchJson(query: trimmed, completionHandler: completion)
            },
            onSuccess: { [weak self] payload in
                guard let self else { return }
    ...
    func login(email: String, password: String, completion: @escaping (Bool) -> Void) {
        ...
        performBridgeJsonRequest(
            call: { callback in
                TvosContentBridge.shared.loginJson(email: email, password: password, completionHandler: callback)
            },
            onSuccess: { [weak self] payload in
                guard let self else {
                    completion(false)
    ...
    private func reconcileLoginState(fallbackEmail: String) -> Bool {
        guard TvosContentBridge.shared.isLoggedIn() else {
            return false
        }

        errorMessage = ""
        isLoggedIn = true
        let bridgedEmail = TvosContentBridge.shared.currentUserEmail()
    ...
    func logout() {
        TvosContentBridge.shared.logout()
        refreshShell(downloadsEnabled: downloadsEnabled)
    }
