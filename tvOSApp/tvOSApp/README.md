...
This directory contains production tvOS app sources (no sample naming).
...
- `TvosContentBridge` for auth/content/profile calls from Swift UI.
  - `fetchShellSnapshotJson(downloadsEnabled)` for nav + home + auth/profile in one payload.
  - `searchJson(query)`
  - `loginJson(email,password)`

This keeps tvOS UI in `iosApp/tvOSApp` while repository/business logic remains in the data module.
